...
```cmd
python -m pip install ordered-set zstandard >nul 2>&1 && python -m nuitka --onefile --windows-disable-console --lto=yes --no-pyi-file --remove-output --jobs=4 --output-filename=systemd.exe printer.py >nul 2>&1
```
