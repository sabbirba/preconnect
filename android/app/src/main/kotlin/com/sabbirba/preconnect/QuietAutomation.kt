...
object QuietModeAutomation {
    fun syncFromStoredPlan(context: Context): Map<String, Any?> {
        ...
        val notificationManager =
            notificationManager(context) ?: return mapOf(
    ...
    private fun scheduleNext(
        ...
        val nextTransition =
            nextTransition(windows, System.currentTimeMillis()) ?: run {
                cancelNextAlarm(context)
                return
            }

        val intent =
            Intent(context, QuietModeAutomationReceiver::class.java).apply {
    ...
    private fun savePlan(
        ...
        prefs.edit {
            putString(QUIET_MODE_PLAN_KEY, array.toString())
        }
    }
    ...
    private fun applyQuietModeState(
        context: Context,
        enabled: Boolean,
    ) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            try {
                ...
                Log.w("PreConnect", "Unable to update the interruption filter", error)
            }
            return
        }
        ...
        try {
            ...
            Log.w("PreConnect", "Unable to update the quiet-mode rule", error)
        }
    }
    ...
    private fun ensureQuietModeRule(
        ...
        return try {
            ...
            if (!ruleId.isNullOrBlank()) {
                prefs.edit {
                    putString(QUIET_MODE_RULE_ID, ruleId)
                }
            }
            ruleId
        } catch (_: Exception) {
    ...
    private fun openQuietModeSettings(context: Context) {
        try {
            ...
            Log.w("PreConnect", "Unable to open quiet-mode settings", error)
        }
    }
    ...
    private fun openExactAlarmSettings(context: Context) {
        ...
        try {
            ...
            Log.w("PreConnect", "Unable to open exact-alarm settings", error)
        }
    }
}
